...
import { DetailAbout } from '../stats/rx_detail_table'
...
export function XcamFull(props: { rm: RouterMain }): React.ReactNode
{
	...
	var board = null;
	if (menuId === 0) {
		board = <XcamBoard rm={ props.rm }  hrefBase={ ["xcam", "", "/xcam/players"] } showStd={ false } beta={ false }/>;
	} else if (menuId === 1) {
		board = <XcamStatBoard hrefBase={ ["/xcam/players", "/xcam"] } slug={ props.rm.core.slug }
			scoreData={ G_SHEET.scoreData }
			/*starMap={ G_SHEET.starMap } userMap={ G_SHEET.userMap } userRankStore={ null }
			altStarMap={ G_SHEET.extStarMap } altMap={ G_SHEET.altMap }*/
			aboutNode={ <DetailAbout/> } key={ readSlug(props.rm.core.slug, "name") }/>;
	}
